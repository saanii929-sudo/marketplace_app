...
enum SessionStatus {
  ...
  /// A valid session, but for a non-customer account — this app is
  /// customer-only, so the session is logged out immediately.
  wrongRole,
}

/// Resolves whether the stored session (if any) belongs to a signed-in
/// customer. This app has no local cache of the user's role, so a stored
/// access token is only ever a hint — `GET /accounts/me/` is the source of
/// truth, called once here via [profileControllerProvider].
